...
(comment
  ...
  (reduce (fn [acc e]
            (if-let [exam-key (first (keys e))]
              (update acc exam-key conj (get e exam-key))
              acc)) {} [{:a 1} {:c 44} {:a 2} {:b 11} {:c 33}])
  ;; => {:a (2 1), :c (33 44), :b (11)}

  (reduce (fn [acc m]
            (let [exam-keys (keys m)]
              (if (empty? exam-keys)
                acc
                (reduce (fn [acc2 k]
                          (update acc2 k conj (get m k))) acc exam-keys))))
          {}
          [{:a 1 :b 2} {:a 11 :b 22} {:c 12 :a \a}])
;; => {:a (\a 11 1), :b (22 2), :c (12)}

  (reduce (fn [acc m]
            (reduce (fn [acc2 k]
                      (update acc2 k conj (get m k))) acc (keys m)))
          {}
          #_[{} {}]
          [{:a 1 :b 2} {:a 11 :b 22} {:c 12 :a \a}])


  (defn exams-results-reducer [acc exam-result]
    (let [exam-keys (keys exam-result)]
      (if (empty? exam-keys)
        acc
        (reduce acc exam-keys))))
